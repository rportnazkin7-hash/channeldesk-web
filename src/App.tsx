...
function isHttpUrl(url:string){return /^https?:\/\//i.test(url.trim())}
...
export default function App(){
 ...
 const forwardPostOpened=useRef(false)

 function applySnapshot(snapshot:Awaited<ReturnType<typeof api.workspaceSnapshot>>){setPending(snapshot.pending);setChannels(snapshot.channels);setMembers(snapshot.members);setPosts(snapshot.posts);setTemplates(snapshot.templates);setAdvertisers(snapshot.advertisers);setBookings(snapshot.bookings);setFeedbackItems(snapshot.feedback);setFinSummary(snapshot.finance_summary);setMediaKits(snapshot.media_kits);setTasks(snapshot.tasks)}
 async function load(){setLoading(true);setError('');try{const s=await api.workspaces();setSpaces(s);const requestedWorkspaceId=Number(new URLSearchParams(window.location.search).get('forward_workspace')||0);const a=s.find(x=>x.id===requestedWorkspaceId)||s.find(x=>x.id===activeId)||s[0]||null;setActiveId(a?a.id:null);if(a){applySnapshot(await api.workspaceSnapshot(a.id))}else{applySnapshot({pending:[],channels:[],members:[],posts:[],templates:[],advertisers:[],bookings:[],feedback:[],finance_summary:null,media_kits:[],tasks:[]})}setOpenPost(null)}catch(e){setError(e instanceof Error?e.message:'Ошибка загрузки')}finally{setLoading(false)}}
 // Тихий фоновый poll: один snapshot-запрос вместо нескольких параллельных запросов.
 const refresh=useCallback(async ()=>{if(refreshInFlight.current)return;refreshInFlight.current=true;try{const s=await api.workspaces();const a=s.find(x=>x.id===activeId)||s[0]||null;if(!a)return;applySnapshot(await api.workspaceSnapshot(a.id));setSpaces(s)}catch{/* фоновая ошибка не должна тревожить пользователя */}finally{refreshInFlight.current=false}},[activeId])
 ...
 async function removeChannel(id:number){buzz();if(!active)return;if(!window.confirm('Убрать канал из этого рабочего пространства? Его можно будет подключить снова.'))return;try{await api.deleteChannel(active.id,id);await load()}catch(e){setError(e instanceof Error?e.message:'Ошибка удаления канала')}}
 ...
 useEffect(()=>{const target=Number(new URLSearchParams(window.location.search).get('forward_post')||0);if(!target||forwardPostOpened.current||!active||!posts.some(post=>post.id===target))return;forwardPostOpened.current=true;setTab('posts');void openDetails(target)},[posts,active?.id])
   ...
   {open&&<div style={{marginTop:12,padding:12,borderRadius:14,background:'var(--panel-2)',border:'1px solid var(--border)'}}>
    ...
    <div style={{marginBottom:12}}>
     ...
     {(assetsByPost[p.id]||[]).length>0&&<div style={{display:'flex',flexWrap:'wrap',gap:8,marginTop:8}}>
      {(assetsByPost[p.id]||[]).map(a=>{const remote=isHttpUrl(a.file_url);const shortName=a.file_name.length>20?a.file_name.slice(0,20)+'…':a.file_name;return a.file_type.startsWith('image/')&&remote?<div key={a.id} style={{position:'relative'}}><img src={a.file_url} alt={a.file_name} style={{width:64,height:64,borderRadius:10,objectFit:'cover',border:'1px solid var(--border-2)'}}/>{editable&&<button onClick={()=>delAsset(p.id,a.id)} style={{position:'absolute',top:-6,right:-6,background:'var(--danger-bg)',border:'1px solid var(--danger-border)',color:'var(--danger)',borderRadius:'50%',width:20,height:20,display:'grid',placeItems:'center',padding:0}}><X size={11}/></button>}</div>:<div key={a.id} style={{display:'flex',alignItems:'center',gap:6,border:'1px solid var(--border-2)',borderRadius:10,padding:'6px 10px',background:'var(--chip-bg)',fontSize:12}}>📎 {remote?<a href={a.file_url} target="_blank" rel="noreferrer" style={{color:'var(--accent-2)',textDecoration:'none'}}>{shortName}</a>:<span>{shortName} · из Telegram</span>}{editable&&<button onClick={()=>delAsset(p.id,a.id)} style={{background:'none',border:0,color:'var(--danger)',padding:0,marginLeft:4}}><X size={12}/></button>}</div>})}
     </div>}
 ...
 return <div className="app"><header><div><span className="eyebrow">РАБОЧЕЕ ПРОСТРАНСТВО</span><h1>ChannelDesk</h1></div><button className="workspace" onClick={()=>{buzz();load()}} title="Обновить"><RefreshCw size={15}/></button></header><main>
  ...
  {!showCompose&&!showTasks&&!showMediaKits&&!showStatistics&&!showAnalytics&&!showCampaignFlow&&!showBookingForm&&!showAdvForm&&!showSettings&&!showIntegrations&&!showPublicNews&&tab==='overview'&&<>
   {!active&&!loading?<section className="hero"><p>Создайте рабочее пространство агентства.</p><input value={name} onChange={e=>setName(e.target.value)} style={{width:'100%',padding:13,borderRadius:12,border:'1px solid var(--border-2)',background:'var(--field-bg)',color:'white',marginBottom:12}}/><button onClick={create}><CirclePlus size={19}/> Создать</button></section>:<>
    ...
    <section id="channels-section" className="panel"><div className="panel-title"><h2>Каналы</h2><CalendarDays size={20}/></div>{channels.length?channels.map(c=><div key={c.id} style={{padding:'15px 0',borderBottom:'1px solid var(--border)',display:'flex',justifyContent:'space-between',alignItems:'center',gap:12}}><div><strong>{c.title}</strong><div style={{color:'var(--accent-2)',fontSize:12}}>● подключён</div></div>{canManage&&<button className="icon-btn danger" onClick={()=>void removeChannel(c.id)} disabled={busy}>Удалить</button>}</div>):<div className="empty"><div className="empty-icon"><Megaphone/></div><h3>Каналов пока нет</h3><p>Добавьте бота администратором канала и обновите экран.</p></div>}</section>
  ...
  {showPublicNews&&<section className="panel">
   <div className="panel-title"><h2>Приём новостей</h2><Inbox size={20}/></div>
   <button className="back-btn" onClick={()=>{buzz();setShowPublicNews(false)}} style={{marginBottom:12}}>← Назад</button>
   {!active?<p style={{color:'var(--muted)'}}>Сначала создайте рабочее пространство.</p>:!canManage?<p style={{color:'var(--muted)'}}>Создать публичную форму может только владелец или администратор.</p>:<>
    <p style={{color:'var(--muted)',fontSize:13,lineHeight:1.5}}>Дайте подписчикам ссылку «Предложить новость». Материал попадёт редактору как черновик и не будет опубликован автоматически.</p>
    <label className="form-label">Канал назначения</label>
    <select className="field" value={newsChannelId??''} onChange={e=>setNewsChannelId(e.target.value?Number(e.target.value):null)}><option value="">Без выбранного канала</option>{channels.map(channel=><option key={channel.id} value={channel.id}>{channel.title}</option>)}</select>
    <label className="form-label">Заголовок страницы</label><input className="field" value={newsPageTitle} onChange={e=>setNewsPageTitle(e.target.value)} placeholder="Предложить новость" />
    <label className="form-label">Описание для читателей</label><textarea className="field" rows={3} value={newsPageDescription} onChange={e=>setNewsPageDescription(e.target.value)} placeholder="Есть новость, фото или видео? Отправьте материал редакции." />
    <button className="primary-btn" onClick={()=>void createPublicNewsPage()} disabled={newsPageBusy||!newsPageTitle.trim()}>{newsPageBusy?'Создаю…':'Создать ссылку приёма новостей'}</button>
    {newsPageUrl&&<div className="invite-box"><p>Ссылка создана и скопирована:</p><code>{newsPageUrl}</code><button className="icon-btn" style={{marginTop:8}} onClick={()=>{buzz();navigator.clipboard?.writeText(newsPageUrl)}}>Скопировать</button><button className="icon-btn danger" style={{marginTop:8,marginLeft:8}} onClick={()=>void deletePublicNewsPage()}>Отключить ссылку</button></div>}
   </>}
  </section>}
