...
export type ChannelMetric={id:number;workspace_id:number;channel_id:number;channel_title?:string;metric_date:string;subscribers:number;views:number;reach:number;reactions:number;forwards:number;posts_count:number;source:'manual'|'bot_api';notes:string}
...
export type AnalyticsOverview={from_date:string;to_date:string;metrics:ChannelMetric[];links:ChannelLink[];summary:AnalyticsSummary}
...
export const api={
 ...
 deleteBooking:(wid:number,id:number)=>req<void>(`/api/workspaces/${wid}/bookings/${id}`,{method:'DELETE'}),
  ...
  createMetric:(wid:number,p:{channel_id:number;metric_date:string;subscribers?:number;views?:number;reach?:number;reactions?:number;forwards?:number;posts_count?:number;source?:'manual'|'bot_api';notes?:string})=>req<ChannelMetric>(`/api/workspaces/${wid}/analytics/metrics`,{method:'POST',body:JSON.stringify(p)}),
