...
def create_post(workspace_id: int, payload: PostCreate, user: dict = Depends(current_user)):
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        cur.execute("""INSERT INTO cd_posts(workspace_id,channel_id,title,text,status,approval_required,scheduled_at,created_by)
        VALUES(%s,%s,%s,%s,%s,%s,%s,%s) RETURNING *""",
                    (workspace_id, payload.channel_id, payload.title.strip(), payload.text,
                     payload.status, payload.approval_required, payload.scheduled_at, user['id']))
        post = cur.fetchone()
        if payload.text:
...
def update_post(workspace_id: int, post_id: int, payload: PostUpdate, user: dict = Depends(current_user)):
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        fields, values = [], []
        for key in ('title', 'text', 'channel_id', 'scheduled_at'):
            if key in data:
