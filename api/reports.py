...
def get_public_report(token: str):
    ...
    with connect() as conn, conn.cursor() as cur:
        ...
        WHERE r.token_hash=%s AND r.is_active=true""", (digest,))
        report = cur.fetchone()
        if not report:
        ...
        cur.execute("""SELECT b.id,b.format,b.cost,b.currency,b.status,b.payment_status,
        b.publish_at,b.delete_at,c.title AS channel_title
